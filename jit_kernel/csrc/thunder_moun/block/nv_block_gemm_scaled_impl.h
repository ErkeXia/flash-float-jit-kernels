...
// Pure in-place snapshot from commit 222da59, before the out-of-place pipeline changes.
...
#define USE_INPALCE_TRI_TRANSPOSE_INPLACE_IMPL 1
...
#if !((defined(USE_INPALCE_TRI_TRANSPOSE_INPLACE_IMPL)) && USE_INPALCE_TRI_TRANSPOSE_INPLACE_IMPL)
        // The simple out-of-place path reuses the beginning of the existing X
        // stage storage after the mainloop has consumed every K tile. A 128x128
        // half tile occupies the same bytes as two 128x128 FP8 X stages.
...
#if !((defined(USE_INPALCE_TRI_TRANSPOSE_INPLACE_IMPL)) && USE_INPALCE_TRI_TRANSPOSE_INPLACE_IMPL)
            // Publish generic-proxy epilogue writes before the lower TMA store.
            nvgpu::arch::tma_store_fence();
#endif
            __syncthreads();
...
#else
                    // NOTE (yiakwy) : simple row-major out-of-place transpose.
                    // The lower TMA store and transpose may read shmem_epilogue
...
#endif // USE_INPALCE_TRI_TRANSPOSE_INPLACE_IMPL

#if !((defined(USE_INPALCE_TRI_TRANSPOSE_INPLACE_IMPL)) && USE_INPALCE_TRI_TRANSPOSE_INPLACE_IMPL)
                    // Publish the completed generic-proxy transpose before
                    // thread 0 starts the upper TMA store from shmem_X.
                    nvgpu::arch::tma_store_fence();
                    __syncthreads();
#endif
...
#if !((defined(USE_INPALCE_TRI_TRANSPOSE_INPLACE_IMPL)) && USE_INPALCE_TRI_TRANSPOSE_INPLACE_IMPL)
                if (threadIdx.x == 0) {
                    // This simple path does not overlap an upper store with the
                    // next task. Drain every output group before shmem_X is
                    // reused as mainloop storage.
                    asm volatile("cp.async.bulk.wait_group 0;\n" ::: "memory");
                }
                FFJK_PROF_END(ffjk::kProfilerEventStoreLower);
#endif
