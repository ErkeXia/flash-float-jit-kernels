...
__device__ __inline__ void tma2d_multicast_load_async(void* smem_ptr/*smem dest*/, void const * tma_desc_ptr/*gmem src*/,
    ...
    uint32_t s_w_bar_addr = __cvta_generic_to_shared(s_w_mbar_ptr);

    tma2d_multicast_load_async(smem_addr, tma_desc_addr, s_w_bar_addr, inner_dim_offset, outter_dim_offset, cluster_mask, cache_hint);
}
