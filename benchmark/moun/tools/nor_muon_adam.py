# Adapted from modded-nanogpt implementaiton with NorMuonAdam for verification purpose
...
if os.environ.get("RANK", None) == None:
    os.environ["RANK"] = "0"
rank = int(os.environ.get("RANK", 0))

if os.environ.get("WORLD_SIZE", None) == None:
...
def sparse_comms_start(idxes_np, N, rank, world, send_idxes_buffer):
    ...
    recv_counts_fut = dist.all_to_all_single(
        recv_counts, send_counts, async_op=True
    ).get_future()
...
# -----------------------------------------------------------------------------
# Muon and Adam optimizer for GPT2 (GPTNano 11 layers, 8 heads, 768 dim, 50304 vocab)
...
class NorMuonAndAdam:
    ...
    def __init__(
        ...
        for name, param in named_params:
            ...
            assert (
                label is not None and label in param_table
            )  # all params must have valid label
    ...
    def _build_param_cfg(self, param: nn.Parameter, label: str):
        ...
        if optim == "adam":
            ...
            if reshape[0] % self.world_size != 0:
                raise ValueError(
                    f"reshape[0]={reshape[0]} must be divisible by world_size"
                )
    ...
    def _init_state(self):
        ...
        for param, p_cfg in self.param_cfgs.items():
            if p_cfg.optim == "adam":
                ...
                print(
                    f"[init] { p_cfg.label} : param.shape={param.shape}, chunk_shape={chunk_shape}, second_mom_shape={second_mom_shape}"
                )
    ...
    def _launch_gather(
        self, param: nn.Parameter, p_slice: Tensor
    ...
    def copy_lm_state_to_embed(self):
        ...
        embed_state["step"] = lm_state[
            "step"
        ]  # Preserve step count for bias correction
    ...
    def step(self, do_adam: bool = True):
        ...
        for label in self.work_order:
            ...
            if p_cfg.comms != "sharded_sparse":
                ...
                grad_chunk = sparse_comms_merge_gradients(
                    param.grad, recv_idxes, recv_vals, rank, world_size
                )

            # Apply update based on optim type
            if p_cfg.optim == "adam":
                print(
                    f"[step] Rank {rank}: {label} adam update, param.shape={param.shape}, grad_chunk.shape={grad_chunk.shape if grad_chunk is not None else None}"
                )
                p_slice = self._adam_update(param, grad_chunk, p_cfg, rank)
            else:
                print(
                    f"[step] Rank {rank}: {label} muon update, param.shape={param.shape}, grad_chunk.shape={grad_chunk.shape if grad_chunk is not None else None}"
                )
        ...
        if (
            do_adam
            and not self.split_embed
            and embed_param is not None
            and lm_param is not None
        ):
    ...
    def _adam_update(
        self, param: nn.Parameter, grad_chunk: Tensor, p_cfg: ParamConfig, rank: int
    ) -> Tensor:
    ...
    def _normuon_update(
        ...
        def _log_shapes(p_shape, m_shape, v_shape, label):
            def wrapper():
                print(
                    f"[Dynamo Runtime A - {label}] p_slice: {p_shape} | mantissa: {m_shape} | v_chunk: {v_shape}",
                    flush=True,
                )
        ...
        if p_cfg.per_matrix_lr_mul is not None:
            ...
            for mat_idx in range(p_cfg.chunk_size):
                self._eff_lr_t.fill_(
                    p_cfg.lr_mul * p_cfg.per_matrix_lr_mul[mat_idx] * p_cfg.lr
                )

                wrapper = _log_shapes(
                    tuple(p_slice.shape),
                    tuple(p_state["mantissa"].shape),
                    tuple(v_chunk.shape),
                    p_cfg.label if "p_cfg" in locals() else "Unknown",
                )

                torch._dynamo.assume_constant_result(wrapper)
                wrapper()
    ...
    def _apply_normuon_variance_reduction(
        v_chunk, second_momentum_buffer, beta2, red_dim
    ):
