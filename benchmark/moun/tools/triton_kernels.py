    ...
    def forward(ctx, x, W1, W2, W1_f8=None, dequant_scale=None, x_f8=None):
        ...
        if W1_f8 is not None:
            ...
            pre, post = linear_relu_square(
                x_flat, W1, a_f8=x_f8, b_f8=W1_f8, dequant_scale_ptr=dequant_scale
            )
        else:
...
class FusedSoftcappedCrossEntropy(torch.autograd.Function):
    ...
    def forward(
        ...
        grad_input = torch.empty(
            (n_rows, n_cols), dtype=torch.float8_e5m2, device=logits.device
        )
        ...
        ctx.save_for_backward(
            logits, targets, mtp_weights, lse, x, lm_head_weight, x_f8, w_f8, grad_input
        )
    ...
    def backward(ctx, grad_output):
        ...
        x_f8_T = torch.empty(
            (x_f8.shape[1], x_f8.shape[0]), dtype=x_f8.dtype, device=x_f8.device
        )
        transpose_copy(x_f8, x_f8_T)  # (768, n_rows) row-major

        grad_input_T = torch.empty(
            (n_cols, n_rows), dtype=grad_input.dtype, device=grad_input.device
        )
